...
## 주요 기능
- **인증**: Kakao OAuth(로컬에서는 Dummy provider 추가) → JWT Access Token(Bearer), Refresh Token은 `refresh_token` httpOnly/secure 쿠키로 발급. `/auth/login`, `/auth/refresh`, `/auth/logout` 제공.
- **호환성**: 기존 Google 연동 계정 조회 호환성을 위해 `GOOGLE` provider 값은 유지하지만, Google OAuth 클라이언트는 기본 비활성화 상태로 둡니다.
...
- JWT(JJJWT), OAuth2 Client, Springdoc OpenAPI, H2(로컬/테스트)
...
- 실행 프로필: `SPRING_PROFILES_ACTIVE=local|prod` (로컬은 Dummy provider + H2 사용, prod는 Docker 환경 변수 사용)
- Google OAuth 재활성화가 필요하면 `oauth.google.enabled=true` 와 함께 `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET` 을 제공합니다.
- 실행 프로필: `SPRING_PROFILES_ACTIVE=local|prod` (로컬은 Dummy provider + H2 사용, prod는 Docker 환경 변수 사용)
